                ...
                for line in input_file:
                    ...
                    if len(columns) >= 27:
                        ...
                        if data_country_name == country_name:
                            if fires and forest_fires and urban_pop and \
                                    rural_pop:  # makes sure we dont grab
                                # empty fields
                                found = True
                                total_fires = float(fires) + float(
                                    forest_fires)
                                years = int(year)
                                ...
                                output_file.write(
                                    f"{year},{total_fires},"
                                    f"{urban_population},{rural_population}\n")

        if not found:
            raise ValueError(
                f"Country '{country_name}' not found in the CSV file.")
